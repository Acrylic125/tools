...
export function ConvertImgToIcoSection() {
  ...
  const convertImageToMultipleIcos = async (file: File): Promise<Blob> => {
    ...
    try {
      ...
      throw new Error(
        `Failed to create ICO files: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  };
  ...
  const handleConvertAndDownload = async () => {
    if (files.length === 0) {
      setConversionError("Please select an image file first");
      return;
    }
    ...
    try {
      const file = files[0];
      const zipBlob = await convertImageToMultipleIcos(file);
